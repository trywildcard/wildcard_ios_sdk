...
public class ArticleCard : Card{
    ...
    */
    public class func search(query:String, limit:Int, completion: (([ArticleCard]?, NSError?)->Void)) -> Void{
        Platform.sharedInstance.generalSearchFromQuery(query, limit:limit, type:Card.stringFromCardType(CardType.WCCardTypeArticle), completion: { (cards:[Card]?, error:NSError?) -> Void in
            if(error != nil){
